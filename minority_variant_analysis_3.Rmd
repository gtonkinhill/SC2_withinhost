...
Plot iSNV counts across the genome

```{r}
#load gene locations
annotations <- as_tibble(genbankr::readGenBank("./data/MN908947.3.gbk")@genes)

pos_maf_count <- temp %>% group_by(pos, mut) %>%
  summarise(
    maf_count = n(),
    freq = n()/length(all_sample_names)
  )

ggplot(pos_maf_count, aes(x=pos, y=maf_count)) + 
  geom_point(size=0.5) +
  scale_y_sqrt() + 
  geom_rect(data=annotations, inherit.aes=FALSE, aes(xmin=start, xmax=end, ymin=0,
                ymax=700, fill=gene), alpha=0.3) +
  geom_label_repel(data=annotations, inherit.aes=FALSE, aes(x=start+(end-start)/2, y=400, label=gene), direction = 'y', segment.alpha = 0, size=3) +
  theme_bw(base_size = 16) +
  xlab("position in Sars-Cov-2 genome") +
  ylab("sample count") 

ggsave(filename = "./Figures/maf_count_w_annotation.pdf", width = 12, height = 7)
ggsave(filename = "./Figures/maf_count_w_annotation.png", width = 12, height = 7)
```

##Distribution of shared iSNVs across consensus phylogeny

we also exclude sites seen in more than 2.5% of samples

```{r}
all_seqs <- ape::read.dna("./data/consensus/MA_elan.20200529.consensus.filt.fasta", format = "fasta")
# dated_tree <- read.tree("./data/consensus/elan.20200529.consensus.filt.mask.dated_rtt.tree")
# dated_tree <- drop.tip(dated_tree, dated_tree$tip.label[!dated_tree$tip.label %in% all_sample_names])
...
#exclude positions seen in more than 2% of samples
...
# plotdf$variable[plotdf$variable=='cons_count'] <- "found in COG-UK consensus genomes"
# plotdf$variable[plotdf$variable=='non_cons_count'] <- "found only as a minor allele"
...
# inigo_method <- c('CAMB-71F0D', 'CAMB-7217F', 'CAMB-72300', 'CAMB-7250A', 'CAMB-75974', 'CAMB-75A26', 'CAMB-75BE7', 'CAMB-75EEE', 'CAMB-76409', 'CAMB-76515', 'CAMB-766E5', 'CAMB-767C4', 'CAMB-768D0', 'CAMB-76AF8', 'CAMB-76B5F', 'CAMB-76F26', 'CAMB-7763F', 'CAMB-77893', 'CAMB-77CB5', 'CAMB-77E28', 'CAMB-77FBC', 'CAMB-78FD9', 'CAMB-79776', 'CAMB-79E71', 'CAMB-79FBA', 'CAMB-7A2FC', 'CAMB-7A57B', 'CAMB-7A599', 'CAMB-7A766')
...
We can also investigate lineage specific mutations as another check for multiple infections.

```{r}
meta <- fread("./data/consensus/20200529_trees/cog_global_2020-05-29_metadata.csv", data.table = FALSE) %>% as_tibble()
meta$sampleID <- map_chr(str_split(meta$sequence_name, '/'), ~ .x[[2]])
msa <- ape::read.dna("./data/consensus/20200529_trees/cog_2020-05-29_alignment.fasta", format = 'fasta')

stopifnot(all(rownames(msa) %in% meta$sequence_name))

lineages <- meta$lineage[match(rownames(msa), meta$sequence_name)]
lineage_count <- table(lineages)

unique_muts <- map_dfr(1:ncol(msa), ~{
  tibble(
    lineage = lineages,
    nt = as.character(msa[,.x])
    ) %>% 
    group_by(nt) %>%
    summarise(
      lineages=list(unique(lineage)),
      count=length(unique(lineage)),
      seq_count=n()
    ) %>%
    filter(count==1) %>%
    filter(nt %in% c('a','c','g','t')) %>%
    add_column(pos=.x, .before=1)
})

unique_muts$lineages <- map_chr(unique_muts$lineages, ~ unlist(.x))
unique_muts_in_all <- unique_muts[unique_muts$seq_count==lineage_count[unique_muts$lineages],]
```

Initially we check for variants at sites that are consistent within a lineage. As not all lineages have such a site this only captures a subset of the data.
...
possible_contams <- map_dfr(run_consensus_df$sample, ~{
  temp <- run_consensus_df %>% filter(sample==.x)
  conflict <- unique_muts_in_all %>% 
    filter(lineages %in% temp$lineage) %>%
    filter(pos %in% temp$POS) %>%
    add_column(sample=.x, .before = 1)
  })
possible_contams[!duplicated(possible_contams$sample),]
```

##Concordance with consensus phylogeny


###Pairwise transmission model

We now run a simple pairwise transmission model on the 1014 consensus genomes with good replicates and dating information. This can then be compared with the minor variant analysis.
...
trans_vs_poly <- map_dfr(c(0.01,0.02, 0.05, 0.1), function(min_maf){
  trans_probs$shared_poly <- map_dbl(1:nrow(trans_probs), function(i){
    # mafA <- samples_list[[trans_probs$sampleA[[i]]]]
    # mafB <- samples_list[[trans_probs$sampleB[[i]]]]
    # 
    # inboth <- mafA$POS[mafA$POS %in% mafB$POS]
    # 
    # if (length(inboth)<1){
    ...
    # mafB <-  mafB[match(inboth, mafB$POS),]
    # 
    # inA <- mafA[,c('A', 'C', 'G', 'T')]>=(min_maf*mafA$Good_depth)
    # inB <- mafB[,c('A', 'C', 'G', 'T')]>=(min_maf*mafB$Good_depth)
    # 
    # return(sum(rowSums(inA & inB)>1))
...
plot with possible mixture excluded
...
mD <- max(Dstat_results$D_stat)

weights <- 1/table(unlist(map(complex_calls, ~.x$mut_pos)))
...
trans_vs_poly <- map_dfr(c(0.01,0.02, 0.05, 0.1), function(min_maf){
  trans_probs$shared_poly <- map_dbl(1:nrow(trans_probs), function(i){
    ...
    temp_pos <- mafA$mut_pos[mafA$mut_pos %in% mafB$mut_pos]
    return(sum(weights[temp_pos]))
    
    # temp_pos <- mafA$pos[mafA$mut_pos %in% mafB$mut_pos]
    # return(mD*length(temp_pos) - sum(Dstat_results$D_stat[Dstat_results$pos %in% temp_pos]))
  })
  trans_probs$min_maf <- min_maf
  return(trans_probs)
})
...
ggsave(filename = "./Figures/pairwise_transmission_prob_vs_multi_maf_weighted.pdf", width = 9, height = 7)
ggsave(filename = "./Figures/pairwise_transmission_prob_vs_multi_maf_weighted.png", width = 9, height = 7)
```
...
##Investigate sites from Oxford paper

Don't see the 4 most common in our data.

```{r}
ox_4_most_common <- c(24156, 22565, 23434, 26524)
run_consensus_df %>% filter(POS %in% ox_4_most_common)
```

major UK lineages identified in the Oxford paper

```{r}
maj_lineage_sites <- c(241, 14408, 23403)

temp <- run_consensus_df %>% 
  filter(POS %in% maj_lineage_sites) %>% 
  arrange(sample)
as.data.frame(temp)

sum(!temp$sample %in% mixture_tests_mm$sampleA)

temp <- temp[!temp$sample %in% mixture_tests_mm$sampleA,]
as.data.frame(temp)
```
